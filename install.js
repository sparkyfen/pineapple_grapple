...
var nodeView = {views: {"all": {"map": "function(doc) {emit(null, doc)}", "reduce": "_count"},"by_client_mac": {"map": "function(doc) {emit(doc.clientMac, doc)}","reduce": "_count"}, "by_ap_mac": {"map": "function(doc) {emit(doc.apMac, doc)}","reduce": "_count"}, "by_id": {"map": "function(doc) {emit(doc._id, doc)}","reduce": "_count"}, "by_ssid": {"map": "function(doc) {emit(doc.ssid, doc)}","reduce": "_count"}}};

db.createDB(settings.couchdb.nodes, function (err, body) {
  if(err && err.status_code !== 412) {
    return console.log(err);
  }
  var users = db.getNodesTable();
